...
# pack="cmdsr" or "rstan": Package cmdstanr or rstan, cmdstanr is the default
#     'iter' in "cmdsr" excludes 'war'
# ■ Output
# fit: Stan output, par: parameter list,
# a: Intercept
...
##########################################################################
Reg <- function(y, X, see=1234, cha=5, war=1000, ite=21000, pack="cmds")
{
  ...
  if (pack == "rstan") {
    fit <- stan(file = scr, data = dat, pars = par,
                seed = see, chains = cha, warmup = war, iter = ite, init = initi)
  } else {
