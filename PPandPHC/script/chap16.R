...
gqcalc3 <- function(ext) {
   print('Partial regression coefficient y2'); print(gqcal(ext$bs[, 2]))
   print('Partial regression coefficient 21'); print(gqcal(ext$bs12))
   print('Direct effect y1 '); print(gqcal(ext$bs[, 1]))
...
fit03 <- rstan::read_stan_csv(csrfit03$output_files()) # Convert to stan format
########### Run with rstan
#fit03 <- stan(model_code = mu_si_reg, data = dataSet03,
#  init = ini, pars = par, seed = 1234, chains = 5, warmup = 1000, iter = 21000)
#save(fit03, file = "./script/obje/out1603")
#load(file = "./script/obje/out1603"); # Load object previously created with stan()
ext03 <- extract(fit03, par);               # Extract random numbers
...
pairs.panels(d01Biology, smooth = F, density = F, ellipses = F, pch = 16, rug = F, cex.axis = 1.0)
dataSet01 <- list(n = nrow(d01Biology), y = d01Biology[, 3], X = d01Biology[, c(1, 2)]) # Input
ini <- function() { initi(y = d01Biology[, 3], x1 = d01Biology[, 1], x2 = d01Biology[, 2]) } # Initial values
########### Run with cmdstanr
#### Use 'mod03' for compilation
csrfit01 <- mod03$sample(data = dataSet01, chains = 5, iter_sampling = 20000,
    init = ini, iter_warmup = 1000, parallel_chains = 5, seed = 1234)  # MCMC
fit01 <- rstan::read_stan_csv(csrfit01$output_files()) # Convert to stan format
########### Run with rstan
#fit01 <- stan(model_code = mu_si_reg, data = dataSet01
#  init = ini, pars = par, seed = 1234, chains = 5, warmup = 1000, iter = 21000)
#save(fit01, file = "./script/obje/out1601")
#load(file = "./script/obje/out1601"); # Load object previously created with stan()
ext01 <- extract(fit01, par);               # Extract random numbers
...
pairs.panels(d02prejudice, smooth=F, density=F, ellipses=F, pch=16, rug=F, cex.axis=1.0)
dataSet02 <- list(n=nrow(d02prejudice), y=d02prejudice[,3], X=d02prejudice[,c(1,2)]) # Input
ini <- function(){ initi(y=d02prejudice[,3], x1=d02prejudice[,1], x2=d02prejudice[,2]) } # Initial values
########### Run with cmdstanr
#### Use 'mod03' for compilation
csrfit02 <- mod03$sample(data = dataSet02, chains = 5, iter_sampling = 20000,
    init = ini, iter_warmup = 1000, parallel_chains = 5, seed = 1234)  # MCMC
fit02 <- rstan::read_stan_csv(csrfit02$output_files()) # Convert to stan format
########### Run with rstan
#fit02 <- stan(model_code = mu_si_reg, data = dataSet02,
#  init = ini, pars = par, seed = 1234, chains = 5, warmup = 1000, iter = 21000)
#save(fit02, file = "./script/obje/out1602")
#load(file = "./script/obje/out1602"); # Load object previously created with stan()
ext02 <- extract(fit02, par);               # Extract random numbers
...
pairs.panels(d04Impression, smooth=F, density=F, ellipses=F, pch=16, rug=F, cex.axis=1.0)
dataSet04 <- list(n=nrow(d04Impression), y=d04Impression[,3], X=d04Impression[,c(1,2)]) # Input
ini <- function(){ initi(y=d04Impression[,3], x1=d04Impression[,1], x2=d04Impression[,2]) } # Initial values
########### Run with cmdstanr
#### Use 'mod03' for compilation
csrfit04 <- mod03$sample(data = dataSet04, chains = 5, iter_sampling = 20000,
    init = ini, iter_warmup = 1000, parallel_chains = 5, seed = 1234)  # MCMC
fit04 <- rstan::read_stan_csv(csrfit04$output_files()) # Convert to stan format
########### Run with rstan
#fit04 <- stan(model_code = mu_si_reg, data = dataSet04,
#  init = ini, pars = par, seed = 1234, chains = 5, warmup = 1000, iter = 21000)
#save(fit04, file = "./script/obje/out1604")
#load(file = "./script/obje/out1604"); # Load object previously created with stan()
ext04 <- extract(fit04, par);               # Extract random numbers
...
ini <- function(){ initi(y=d05Yield[,3], x1=d05Yield[,1], x2=d05Yield[,2]) } # Initial values
########### Run with cmdstanr
#### Use 'mod03' for compilation
csrfit05 <- mod03$sample(data = dataSet05, chains = 5, iter_sampling = 20000,
    init = ini, iter_warmup = 1000, parallel_chains = 5, seed = 1234)  # MCMC
fit05 <- rstan::read_stan_csv(csrfit05$output_files()) # Convert to stan format
########### Run with rstan
#fit05 <- stan(model_code = mu_si_reg, data = dataSet05,
#  init = ini, pars = par, seed = 1234, chains = 5, warmup = 1000, iter = 21000)
#save(fit05, file = "./script/obje/out1605")
#load(file = "./script/obje/out1605"); # Load object previously created with stan()
ext05 <- extract(fit05, par);               # Extract random numbers
...
ini <- function(){ initi(y=d06Grade8[,3], x1=d06Grade8[,1], x2=d06Grade8[,2]) } # Initial values
########### Run with cmdstanr
#### Use 'mod03' for compilation
csrfit06 <- mod03$sample(data = dataSet06, chains = 5, iter_sampling = 20000,
    init = ini, iter_warmup = 1000, parallel_chains = 5, seed = 1234)  # MCMC
fit06 <- rstan::read_stan_csv(csrfit06$output_files()) # Convert to stan format
########### Run with rstan
#fit06 <- stan(model_code = mu_si_reg, data = dataSet06,
#  init = ini, pars = par, seed = 1234, chains = 5, warmup = 1000, iter = 21000)
#save(fit06, file = "./script/obje/out1606")
#load(file = "./script/obje/out1606"); # Load object previously created with stan()
ext06 <- extract(fit06, par);               # Extract random numbers
...
pairs.panels(d07Loneliness, smooth=F, density=F, ellipses=F, pch=16, rug=F, cex.axis=1.0)
dataSet07 <- list(n=nrow(d07Loneliness), y=d07Loneliness[,3], X=d07Loneliness[,c(1,2)]) # Input
ini <- function(){ initi(y=d07Loneliness[,3], x1=d07Loneliness[,1], x2=d07Loneliness[,2]) } # Initial values
########### Run with cmdstanr
#### Use 'mod03' for compilation
csrfit07 <- mod03$sample(data = dataSet07, chains = 5, iter_sampling = 20000,
    init = ini, iter_warmup = 1000, parallel_chains = 5, seed = 1234)  # MCMC
fit07 <- rstan::read_stan_csv(csrfit07$output_files()) # Convert to stan format
########### Run with rstan
#fit07 <- stan(model_code = mu_si_reg, data = dataSet07,
#  init = ini, pars = par, seed = 1234, chains = 5, warmup = 1000, iter = 21000)
#save(fit07, file = "./script/obje/out1607")
#load(file = "./script/obje/out1607"); # Load object previously created with stan()
ext07 <- extract(fit07, par);               # Extract random numbers
######## Figure 16.21 Path diagram (7)
gqcalc3(ext = ext07)
######## Figure 16.20 3D scatter plot regression plane (7)
plot3d(d07Loneliness[,1], d07Loneliness[,2], d07Loneliness[,3], size = 3, type = "p",
       xlab = "z1", ylab = "z2", zlab = "z3") 
...
ini <- function(){ initi(y=d08Height[,3], x1=d08Height[,1], x2=d08Height[,2]) } # Initial values
########### Run with cmdstanr
#### Use 'mod03' for compilation
csrfit08 <- mod03$sample(data = dataSet08, chains = 5, iter_sampling = 20000,
    init = ini, iter_warmup = 1000, parallel_chains = 5, seed = 1234)  # MCMC
fit08 <- rstan::read_stan_csv(csrfit08$output_files()) # Convert to stan format
########### Run with rstan
#fit08 <- stan(model_code = mu_si_reg, data = dataSet08,
#  init = ini, pars = par, seed = 1234, chains = 5, warmup = 1000, iter = 21000)
#save(fit08, file ="./script/obje/out1608")
#load(file = "./script/obje/out1608"); # Load object previously created with stan()
ext08 <- extract(fit08, par);               # Extract random numbers
...
pairs.panels(d09WritingTest, smooth=F, density=F, ellipses=F, pch=16, rug=F, cex.axis=1.0)
dataSet09 <- list(n=nrow(d09WritingTest), y=d09WritingTest[,3], X=d09WritingTest[,c(1,2)]) # Input
ini <- function(){ initi(y=d09WritingTest[,3], x1=d09WritingTest[,1], x2=d09WritingTest[,2]) } # Initial values
########### Run with cmdstanr
#### Use 'mod03' for compilation
csrfit09 <- mod03$sample(data = dataSet09, chains = 5, iter_sampling = 20000,
    init = ini, iter_warmup = 1000, parallel_chains = 5, seed = 1234)  # MCMC
fit09 <- rstan::read_stan_csv(csrfit09$output_files()) # Convert to stan format
########### Run with rstan
#fit09 <- stan(model_code = mu_si_reg, data = dataSet09,
#  init = ini, pars = par, seed = 1234, chains = 5, warmup = 1000, iter = 21000)
#save(fit09, file = "./script/obje/out1609")
#load(file = "./script/obje/out1609"); # Load object previously created with stan()
ext09 <- extract(fit09, par);               # Extract random numbers
######## Figure 16.27 Path diagram (9)
gqcalc3(ext = ext09)
######## Figure 16.26 3D scatter plot regression plane (9)
plot3d(d09WritingTest[,1], d09WritingTest[,2], d09WritingTest[,3], size = 3, type = "p",
       xlab = "z1", ylab = "z2", zlab = "z3") 
