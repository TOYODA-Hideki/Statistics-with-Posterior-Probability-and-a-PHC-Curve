...
plot2gequ<-function(y,x,a,ext,ba=5,xlab="pretest"){
  plot(x,y,pch=ifelse(a-1,1,16),xlab=xlab,ylab="postest",
       cex.lab=1.5,cex.axis=1.5,xlim=c(min(x)-ba,max(x)+ba),
       ylim=c(min(y)-ba,max(y)+ba))
  abline(mean(ext$a[,1]),mean(ext$b))     
  abline(mean(ext$a[,2]),mean(ext$b))     
}
# Sample variance
...
plot2gdef<-function(y,x,a,ext,ba=3){
  plot(x,y,pch=ifelse(a-1,1,16),xlab="pretest",ylab="posttest",
       cex.lab=1.5,cex.axis=1.5,xlim=c(min(x)-ba,max(x)+ba),
       ylim=c(min(y)-ba,max(y)+ba))
  abline(mean(ext$a[,1]),mean(ext$b[,1]))     
  abline(mean(ext$a[,2]),mean(ext$b[,2]))     
}
...
plot2gdefdef<-function(x,ext){
  ...
  yhat_sa<-matrix(0,nrow(ext$a),leng)
  yhat_sa_HL<-matrix(0,leng,2)
  for (i in (1:leng)){
    yhat_sa[,i]<-ext$a[,1]-ext$a[,2]+(ext$b[,1]-ext$b[,2])*xlabe[i]}
  for (i in (1:leng)){
  ...
  plot(xlabe,yhat_sa_HL[,2],ylim=c(min(yhat_sa_HL),max(yhat_sa_HL)),
       type="l",ylab="Difference in predicted values for post",xlab="pre value",lty=2,lwd=2.0)
}
...
fitequ01 <- rstan::read_stan_csv(csrfitequ01$output_files()) # Conversion to Stan format

########### Execution with rstan
#fitequ01<-stan(model_code =ANCOVAequ, pars=par, data=dataSetequ01,
#  seed=1234, chains=5, warmup=1000,  iter=21000)
#save(fitequ01,  file="./script/obje/out2001")
#load(file="./script/obje/out2001"); # Load the object previously made with stan( )

extequ01<-extract(fitequ01, par);               # Extracting random numbers

##### Table 20.2 Summary of Posterior Distribution of Parameters for "Data1" with Common Slope Model
gqcal(extequ01$a[,1],2)
gqcal(extequ01$a[,2],2)
gqcal(extequ01$b, 2)
...
a_sa01<-extequ01$a[,1]-extequ01$a[,2];                # Posterior distribution of the difference in intercepts
gqcal(a_sa01,1);                                      
...
csrfitequ02<-modequ01$sample(data=dataSetequ02, chains=5, iter_sampling=20000,
                             iter_warmup = 1000, parallel_chains = 5, seed=1234)  # MCMC
fitequ02 <- rstan::read_stan_csv(csrfitequ02$output_files()) # Conversion to Stan format

########### Execution with rstan
#fitequ02<-stan(model_code =ANCOVAequ, pars=par,
#  data=list,
#  seed=1234, chains=5, warmup=1000,  iter=21000)
#save(fitequ02, file="./script/obje/out2002")
#load(file="./script/obje/out2002"); # Load the object previously made with stan( )

extequ02<-extract(fitequ02, par);               # Extracting random numbers

##### Table 20.4 Summary of Posterior Distribution of Parameters for "Data2" with Common Slope Model
gqcal(extequ02$a[,1],2)
gqcal(extequ02$a[,2],2)
gqcal(extequ02$b, 2)
...
a_sa02<-extequ02$a[,1]-extequ02$a[,2];                   # Posterior distribution of the difference in intercepts
gqcal(a_sa02,1);                                         
...
csrfitdef03<-moddef03$sample(data=dataSetdef03, chains=5, iter_sampling=20000,
                             iter_warmup = 1000, parallel_chains = 5, seed=1234)  # MCMC
fitdef03 <- rstan::read_stan_csv(csrfitdef03$output_files()) # Conversion to Stan format

########### Execution with rstan
#fitdef03<-stan(model_code =ANCOVAdef, pars=par, data=dataSetdef03,
#  seed=1234, chains=5, warmup=1000, iter=21000)
#save(fitdef03, file="./script/obje/out2003")
#load(file="./script/obje/out2003"); # Load the object previously made with stan( )

extdef03<-extract(fitdef03, par);               # Extracting random numbers

###### Table 20.6 Summary of Posterior Distribution of Parameters for "Data3" with Different Slope Model
gqcal(extdef03$a[,1],2)
gqcal(extdef03$a[,2],2)
gqcal(extdef03$b[,1],2)
gqcal(extdef03$b[,2],2)
...
csrfitdef01<-moddef03$sample(data=dataSetequ01, chains=5, iter_sampling=20000,
                             iter_warmup = 1000, parallel_chains = 5, seed=1234)  # MCMC
fitdef01 <- rstan::read_stan_csv(csrfitdef01$output_files()) # Conversion to Stan format

########### Execution with rstan
#fitdef01<-stan(model_code =ANCOVAdef, pars=par, data=dataSetequ01,
#  seed=1234, chains=5, warmup=1000, iter=21000)
#save(fitdef01, file="./script/obje/out2004")
#load(file="./script/obje/out2004"); # Load the object previously made with stan( )

extdef01<-extract(fitdef01, par);               # Extracting random numbers

#### Summary of Posterior Distribution of Parameters for "Data1" with Different Slope Model (not in the textbook)
gqcal(extdef01$a[,1],2)
gqcal(extdef01$a[,2],2)
gqcal(extdef01$b[,1],2)
gqcal(extdef01$b[,2],2)
...
csrfitdef02<-moddef03$sample(data=dataSetequ02, chains=5, iter_sampling=20000,
                             iter_warmup = 1000, parallel_chains = 5, seed=1234)  # MCMC
fitdef02 <- rstan::read_stan_csv(csrfitdef02$output_files()) # Conversion to Stan format

########### Execution with rstan
#fitdef02<-stan(model_code =ANCOVAdef, pars=par, data=dataSetequ02,
#  seed=1234, chains=5, warmup=1000, iter=21000)
#save(fitdef02, file="./script/obje/out2005")
#load(file="./script/obje/out2005"); # Load the object previously made with stan( )

extdef02<-extract(fitdef02, par);               # Extracting random numbers
...
b_sa01<-extdef01$b[,1]-extdef01$b[,2]
gqcal(b_sa01)
b_sa02<-extdef02$b[,1]-extdef02$b[,2]
gqcal(b_sa02)
b_sa03<-extdef03$b[,1]-extdef03$b[,2]
gqcal(b_sa03)
...
fitpropen01 <- rstan::read_stan_csv(csrfitpropen01$output_files()) # Convert to Stan format

########### Execution with rstan
#fitpropen01<-stan(model_code =propensity01, pars=par, data=dataSetpropen01,
#  seed=1234, chains=5, warmup=1000, iter=21000)
#save(fitpropen01, file="./script/obje/out2006")
#load(file="./script/obje/out2006"); # Load the object previously made with stan( )

extpropen01<-extract(fitpropen01, par);               # Extracting random numbers
...
gqcal(extpropen01$a[,1],2)
gqcal(extpropen01$a[,2],2)
gqcal(extpropen01$b, 2)
gqcal(extpropen01$sigma,2)
gqcal(extpropen01$alpha,2)
gqcal(extpropen01$beta[,1],2)
gqcal(extpropen01$beta[,2],2)
gqcal(extpropen01$beta[,3],2)
gqcal(extpropen01$beta[,4],2)
gqcal(extpropen01$propensity[,1],2)
gqcal(extpropen01$propensity[,2],2)
gqcal(extpropen01$propensity[,3],2)
gqcal(extpropen01$propensity[,4],2)

###### Figure 20.14 Histogram of Propensity Scores by Group
propen01<-apply(extpropen01$propensity, 2, mean);       # EAP of propensity scores
classes<-seq(0, 1, 0.1)
...
a_sa04<-extpropen01$a[,1]-extpropen01$a[,2]
gqcal(a_sa04, 1);
