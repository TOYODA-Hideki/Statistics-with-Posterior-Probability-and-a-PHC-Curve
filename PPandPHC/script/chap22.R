...
plot(X$Actual.Measurement,X$Visual.Estimation,pch=X$Student,xlab="Actual Measurement",ylab="Visual Estimation")
...
plot(Epl11$Actual.Measurement,Epl11$Visual.Estimation,pch=Epl11$Student,   # Figure22.2
...
fit81 <- rstan::read_stan_csv(csrfit81$output_files()) #Conversion to stan format

########### Execution by rstan
#fit81<-stan(model_code =HLM01,data=dataSet81,
#  pars=par, seed=1234, chains=5, warmup=1000,  iter=21000)
#save(fit81,  file="./script/obje/out2201")
#load(file="./script/obje/out2201");   #Load object created with stan( ) in advance

ext81<-extract(fit81, par);               #Extracting random numbers

###### Table22.2 Summary of Posterior Distribution of Parameters for Models with Distributions for Intercepts 
...
av<-apply(ext81$a,2,mean)
bv<-apply(ext81$b,2,mean)
round(rbind(av,bv),2)
...
plot(X$Junior.High.School,X$High.School,pch=X$School,xlab="Junior High School 3rd Year Deviation Value",
...
plot(Chper$Junior.High.School,Chper$High.School,pch=Chper$School,xlab="Junior High 3rd Year Deviation Value",
      ylab="High School First-Year Deviation Value", xlim=c(25,75),ylim=c(25,75))
...
csrfit821 <- mod81$sample(data = dataSet821,chains = 5,iter_sampling = 20000,
    iter_warmup = 1000,parallel_chains = 5,seed=1234)  #MCMC
fit821 <- rstan::read_stan_csv(csrfit821$output_files()) #Conversion to stan format

########### Execution by rstan
#fit821<-stan(model_code =HLM01,data=dataSet821,
#  pars=par, seed=1234, chains=5, warmup=1000,  iter=21000)
#save(fit821,  file="./script/obje/out2202")
#load(file="./script/obje/out2202"); #Load object created with stan( ) in advance

ext821<-extract(fit821, par);               #Extracting random numbers
...
av<-apply(ext821$a,2,mean)
bv<-apply(ext821$b,2,mean)
round(rbind(av,bv),2)

##### Figure 22.8: Ten Single Regression Lines by High School
plot(Chper$Junior.High.School,Chper$High.School,pch=Chper$School,xlab="Junior High Third-Year Deviation Value",
     ylab="High School First-Year Deviation Value", xlim=c(25,75),ylim=c(25,75),type="n")
...
hist(ext821$s_b,breaks=100,main="",ylab="",xlab="Standard deviation of regression coefficients")
...
csrfit822 <- mod822$sample(data = dataSet822,chains = 5,iter_sampling = 20000,
    iter_warmup = 1000,parallel_chains = 5,seed=1234)  #MCMC
fit822 <- rstan::read_stan_csv(csrfit822$output_files()) #Conversion to stan format

########### Execution by rstan
#fit822<-stan(model_code =HLM02,data=dataSet822,
#  pars=par, seed=1234, chains=5, warmup=1000,  iter=21000)
#save(fit822,  file="./script/obje/out2203")
#load(file="./script/obje/out2203"); #Load object created with stan( ) in advance

ext822<-extract(fit822, par);             #Extracting random numbers

###### Table 22.5 Summary of the posterior distribution of the population of the model 
######       with distribution on the intercept
gqcal(ext822$sigma )
gqcal(ext822$a0    )
gqcal(ext822$b     )
gqcal(ext822$s_a   )
...
PHC02(0,ext822$a,digits=2)

###### Figure 22.10 Probability that the difference in intercepts between high school 8 and 7 is greater than c
PHC01(seq(2,8,0.2),ext822$a[,8],ext822$a[,7],xlab="Difference between intercepts for high schools 8 and 7")
...
csrfit91 <- mod81$sample(data = dataSet91,chains = 5,iter_sampling = 20000,
    iter_warmup = 1000,parallel_chains = 5,seed=1234)  #MCMC
fit91 <- rstan::read_stan_csv(csrfit91$output_files()) #Conversion to stan format

########### Execution by rstan
#fit91<-stan(model_code =HLM01,data=dataSet91,
#  pars=par, seed=1234, chains=5, warmup=1000,  iter=21000)
#save(fit91,  file="./script/obje/out2204")
#load(file="./script/obje/out2204"); #Load object created with stan( ) in advance

ext91<-extract(fit91, par);               #Extracting random numbers
...
##### EAP estimates of intercept and regression coefficients
av<-apply(ext91$a,2,mean)
bv<-apply(ext91$b,2,mean)
round(rbind(av,bv),2)
...
csrfit92 <- mod92$sample(data = dataSet92,chains = 5,iter_sampling = 20000,
    iter_warmup = 1000,parallel_chains = 5,seed=1234)  #MCMC
fit92 <- rstan::read_stan_csv(csrfit92$output_files()) #Conversion to stan format

########### Execution by rstan
#fit92<-stan(model_code =HLM03,data=dataSet92,
#  pars=par, seed=1234, chains=5, warmup=1000,  iter=21000)
#save(fit92,  file="./script/obje/out2205")
#load(file="./script/obje/out2205"); #Load object created with stan( ) in advance

ext92<-extract(fit92, par);               #Extracting random numbers
...
##### EAP estimates of intercept and regression coefficients
av<-apply(ext92$ab[,,1],2,mean)
bv<-apply(ext92$ab[,,2],2,mean)
round(rbind(av,bv),2)
...
csrfit93 <- mod93$sample(data = dataSet93,chains = 5,iter_sampling = 20000,
    iter_warmup = 1000,parallel_chains = 5,seed=1234)  #MCMC
fit93 <- rstan::read_stan_csv(csrfit93$output_files()) #Conversion to stan format

########### Execution by rstan
#fit93<-stan(model_code =HLM04,data=dataSet93,
#  pars=par, seed=1234, chains=5, warmup=1000,  iter=21000)
#save(fit93,  file="./script/obje/out2206")
#load(file="./script/obje/out2206"); #Load object created with stan( ) in advance

ext93<-extract(fit93, par);               #Extracting random numbers
...
#### EAP estimates of intercept and regression coefficients
av<-apply(ext93$a,2,mean)
bv<-apply(ext93$b,2,mean)
round(rbind(av,bv),2)
...
csrfit94 <- mod94$sample(data = dataSet94,chains = 5,iter_sampling = 20000,
    iter_warmup = 1000,parallel_chains = 5,seed=1234)  #MCMC
fit94 <- rstan::read_stan_csv(csrfit94$output_files()) #Conversion to stan format

########### Execution by rstan
#fit94<-stan(model_code =HLM05,data=dataSet94,
#  pars=par, seed=1234, chains=5, warmup=1000,  iter=21000)
#save(fit94,  file="./script/obje/out2207")
#load(file="./script/obje/out2207"); #Load object created with stan( ) in advance

ext94<-extract(fit94, par);               #Extracting random numbers
