...
#printIJ(x, IJ): Probability that a conjunctive proposition is true, only with reference point 0
# x : A size[, a, a] 3-dimensional 0/1 array, where 1 indicates row i is greater than column j, and 0 otherwise.
#     Note that this is not a matrix of random numbers but a 0/1 matrix.
# IJ: A 2-column matrix (the first column is rows, the second column is columns), the number of rows is the number of conjunctive propositions
# Probability that μ1 > μ2 > μ3 is true, first checking the necessary condition with c1 = c2 = 0
# Probability that the conjunctive proposition related to Hypothesis C is true (checking the necessary condition)
printIJ(U2, IJ = rbind(c(1, 2), c(2, 3)));
...
# 5)
printIJ(out1003$U2, 2, IJ = rbind(c(2, 3), c(3, 1)))
