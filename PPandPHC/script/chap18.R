...
fitbomb <- rstan::read_stan_csv(csrfitbomb$output_files())  #Conversion to stan format

########### Execution by rstan
#fitbomb<-stan(model_code =Poisson, data=dataSetbomb,
#  pars=par, seed=1234, chains=5, warmup=1000,  iter=21000)
#save(fitbomb,  file="./script/obje/out1801")
#load(file="./script/obje/out1801");       #Load object created with stan( ) in advance

extbomb<-extract(fitbomb, par);               #Extracting random numbers
...
csrfitabsent  <- modbomb$sample(data = dataSetabsent,chains = 5,iter_sampling = 20000,
    iter_warmup = 1000,parallel_chains = 5,seed=1234)               #MCMC
fitabsent  <- rstan::read_stan_csv(csrfitabsent $output_files())   #Conversion to stan format

########### Execution by rstan
#fitabsent<-stan(model_code =Poisson,data=dataSetabsent,
#  pars=par, seed=1234, chains=5, warmup=1000,  iter=21000)
#save(fitabsent,  file="./script/obje/out1802")
#load(file="./script/obje/out1802"); #Load object created with stan( ) in advance

extabsent<-extract(fitabsent, par);               #Extracting random numbers
...
csrfitmirror2 <- modmirror2$sample(data = dataSetmirror2,chains = 5,iter_sampling = 20000,
    iter_warmup = 1000,parallel_chains = 5,seed=1234)      #MCMC
fitmirror2 <- rstan::read_stan_csv(csrfitmirror2$output_files())   #Conversion to stan format

########### Execution by rstan
#fitmirror2<-stan(model_code =Poisson2, data=dataSetmirror2,
#  pars=par, seed=1234, chains=5, warmup=1000,  iter=21000)
#save(fitmirror2,  file="./script/obje/out1803")
#load(file="./script/obje/out1803"); #Load object created with stan( ) in advance

extmirror2<-extract(fitmirror2, par);               #Extracting random numbers
...
csrfitregression <- modregression$sample(data = dataSetregression, chains=5,iter_sampling=20000,
    iter_warmup = 1000,parallel_chains = 5,seed=1234)        #MCMC
fitregression <- rstan::read_stan_csv(csrfitregression$output_files())   #Conversion to stan format

###########Execution by rstan
#fitregression<-stan(model_code =Poisson_reg, data=dataSetregression, #data
#  pars=par, seed=1134, chains=1, warmup=1000,  iter=101000)
#save(fitregression,  file="./script/obje/out1804")
#load(file="./script/obje/out1804");       #Load object created with stan( ) in advance

extregression<-extract(fitregression, par);               #Extracting random numbers
...
gqcal(extregression$lambda[,7],3)
gqcal(extregression$yaste[,7],3)
gqcal(expb,3)

###### Figure 18.5: Poisson Regression of the Number of Deviations
lambda_i<-apply(extregression$lambda,2,mean)
...
fitcontable01 <- rstan::read_stan_csv(csrfitcontable01$output_files()) #Conversion to stan format

###########Execution by rstan
#fitcontable01<-stan(model_code =Poisson_Cross01,data=dataSetcontable01,
#  pars=par, seed=1234, chains=5, warmup=1000,  iter=21000)
#save(fitcontable01,  file="./script/obje/out1805")
#load(file="./script/obje/out1805"); #Load object created with stan( ) in advance

extcontable01<-extract(fitcontable01, par);               #Extracting random numbers
...
csrfitcontable02 <- modcontable02$sample(data = dataSetcontable02,chains = 5,
  iter_sampling=20000, iter_warmup=1000, parallel_chains=5, seed=1234)
fitcontable02 <- rstan::read_stan_csv(csrfitcontable02$output_files()) #Conversion to stan format

###########Execution by rstan
#fitcontable02<-stan(model_code =Poisson_Cross02,data=dataSetcontable02,
#  pars=par, seed=1234, chains=5, warmup=1000,  iter=21000)
#save(fitcontable02,  file="./script/obje/out1806")
#load(file="./script/obje/out1806"); #Load object created with stan( ) in advance

extcontable02<-extract(fitcontable02, par);               #Extracting random numbers
...
print(fitcontable02,digits_summary=3,prob=prob)

###Table 18.18: Probability of Levels & Interaction Effects Being Greater (or Smaller) Than Zero
for (i in 1:4){                                       #Table1 18.18 left
  print(round(mean(extcontable02$A[,i]>0),3))
  print(round(1-mean(extcontable02$A[,i]>0),3))
}
  print(round(mean(extcontable02$B[,1]>0),3))           #Table1 18.18 center
  print(round(1-mean(extcontable02$B[,1]>0),3))
for (i in 1:4){
  print(round(mean(extcontable02$AB[,i,1]>0),3))        #Table 18.18 right
  print(round(1-mean(extcontable02$AB[,i,1]>0),3))
}

###Table 18.19: PHC of Row Proportions Being Greater Than Column Proportions
phc02(0,cbind(extcontable02$p[,1,],extcontable02$p[,2,],
              extcontable02$p[,3,],extcontable02$p[,4,]))


################# Practical Assignment
p_11<-extcontable02$p[,1,1]
p_31<-extcontable02$p[,3,1]
gqcal(p_11)
gqcal(p_31)
