...
fit_1 <- rstan::read_stan_csv(csrfit_1$output_files()) # Conversion to stan format

########### Execution with rstan
#fit_1<-stan(model_code =lognormal01,data=dataSet_1,
#  pars=par, seed=1234, chains=5, warmup=1000,  iter=21000)
#save(fit_1,  file="./script/obje/out1901")
#load(file="./script/obje/out1901"); # Load an object created earlier with stan()

ext_1<-extract(fit_1, par);               # Extraction of random numbers
...
csrfit_2 <- mod_2$sample(data = dataSet_2,chains = 5,iter_sampling = 20000,
                         iter_warmup = 1000,parallel_chains = 5,seed=1234)  # MCMC
fit_2 <- rstan::read_stan_csv(csrfit_2$output_files()) # Conversion to stan format


########### Execution with rstan
#fit_2<-stan(model_code =lognormal02,data=dataSet_2,
#  pars=par, seed=1234, chains=5, warmup=1000, iter=21000)
#save(fit_2, file="./script/obje/out1902")
#load(file="./script/obje/out1902"); # Load an object created earlier with stan()

ext_2<-extract(fit_2, par);               # Extraction of random numbers

###### Table 19.4 Summary of the Posterior Distribution of Parameters and Generated Quantities for Occupation II
gqcal(ext_2$mu[,2])
gqcal(ext_2$sigma[,2])
gqcal(ext_2$heikin[,2] ,0)
gqcal(ext_2$tyuouti[,2],0)
gqcal(ext_2$saihin[,2] ,0)
gqcal(ext_2$sigma0[,2] ,0)
...
gqcal(ext_2$heikin[,2]-ext_2$heikin[,1])
gqcal(ext_2$tyuouti[,2]-ext_2$tyuouti[,1])
gqcal(ext_2$saihin[,2]-ext_2$saihin[,1])

###### Table 19.6 Probability of Difference in Mean, Median, and Mode Values
sequ66<-c(0,10,20,30)
PHC01(sequ66,ext_2$heikin[,2] ,ext_2$heikin[,1] ,cc="gtc",byoga="no")
PHC01(sequ66,ext_2$tyuouti[,2],ext_2$tyuouti[,1],cc="gtc",byoga="no")
PHC01(sequ66,ext_2$saihin[,1] ,ext_2$saihin[,2] ,cc="gtc",byoga="no")
...
PHC01(sequ67,ext_2$heikin[,2] ,ext_2$heikin[,1] ,cc="rope",byoga="no")
PHC01(sequ67,ext_2$tyuouti[,2],ext_2$tyuouti[,1],cc="rope",byoga="no")
PHC01(sequ67,ext_2$saihin[,1] ,ext_2$saihin[,2] ,cc="rope",byoga="no")
...
csrfitlogn <- modlogn$sample(data=dataSetlogn,chains=5,iter_sampling = 20000,
                             iter_warmup = 1000,parallel_chains = 5,seed=1234)  # MCMC
fitlogn <- rstan::read_stan_csv(csrfitlogn$output_files()) # Conversion to stan format

########### Execution with rstan
#fitlogn<-stan(model_code =one_factor_lognormal,data=dataSet02,
#  pars=par, seed=1234, chains=5, warmup=1000, iter=21000)
#save(fitlogn, file="./script/obje/out1903")
#load(file="./script/obje/out1903"); # Load an object created earlier with stan()

extlogn<-extract(fitlogn, par);               # Extraction of random numbers

##### Table 19.10 Posterior Distribution of Parameters of the One-factor Log-normal Distribution Model
print(fitlogn,digits_summary=3,prob=prob)
...
round(mean((extlogn$mu0[,1]>extlogn$mu0[,2])&
             (extlogn$mu0[,2]>extlogn$mu0[,3])),3)    # (Formulas 19.17 and 19.18)
...
csrfitpo <- modpo$sample(data = dataSetpo, chains = 5, iter_sampling = 20000,
                         iter_warmup = 1000, parallel_chains = 5, seed=1234)  # MCMC
fitpo <- rstan::read_stan_csv(csrfitpo$output_files()) # Conversion to stan format

########### Execution with rstan
#fitpo<-stan(model_code =one_factor_Poisson, data=dataSet02,
#  pars=par, seed=1234, chains=5, warmup=1000, iter=21000)
#save(fitpo, file="./script/obje/out1904")
#load(file="./script/obje/out1904"); # Load an object created earlier with stan()

extpo<-extract(fitpo, par);               # Extraction of random numbers

##### Table 19.12 Posterior Distribution of Parameters of the One-factor Poisson Distribution Model
print(fitpo, digits_summary=3, prob=prob)
...
phc02(0, extpo$lambda)

## Probability that the Rest Group Takes Longer than the Non-dominant Hand Group, and the Non-dominant Hand Group Takes Longer than the Dominant Hand Group
round(mean((extpo$lambda[,1]>extpo$lambda[,2]) &
             (extpo$lambda[,2]>extpo$lambda[,3])),3)    # (Formula 19.20)
...
csrfitbi <- modbi$sample(data = dataSetbi, chains = 5, iter_sampling = 20000,
                         iter_warmup = 1000, parallel_chains = 5, seed=1234)  # MCMC
fitbi <- rstan::read_stan_csv(csrfitbi$output_files()) # Conversion to stan format

########### Execution with rstan
#fitbi<-stan(model_code =one_factor_binom, data=dataSetbi, 
#  pars=par, seed=1234, chains=5, warmup=1000, iter=21000)
#save(fitbi, file="./script/obje/out1905")
#load(file="./script/obje/out1905"); # Load an object created earlier with stan()

extbi<-extract(fitbi, par);               # Extraction of random numbers

###### Table 19.15 Posterior Distribution of Parameters of the One-factor Binomial Distribution Model
print(fitbi, digits_summary=3, prob=prob)
...
round(mean((extbi$p[,1]<extbi$p[,2])&(extbi$p[,2]<extbi$p[,3])), 3)
...
csrfitnorm <- modnorm$sample(data=dataSetnorm, chains=5, iter_sampling = 20000,
                             iter_warmup = 1000, parallel_chains = 5, seed=1234)  # MCMC
fitnorm <- rstan::read_stan_csv(csrfitnorm$output_files()) # Conversion to stan format

########### Execution with rstan
#fitnorm<-stan(model_code =one_factor_normal, data=dataSetnorm,
#  pars=par, seed=1234, chains=5, warmup=1000, iter=21000)
#save(fitnorm, file="./script/obje/out1906")
#load(file="./script/obje/out1906"); # Load an object created earlier with stan()

extnorm<-extract(fitnorm, par);               # Extraction of random numbers

##### Table 19.18 Summary of Posterior Distribution of Parameters of the Variance Model
print(fitnorm, digits_summary=3, prob=prob)
...
c1<-c(0,1,2)
c2<-c(0,1,2)
rengen_01<-matrix(0,length(c1),length(c2));
ii<-0;
for (i in c1){
  ii<-ii+1
  jj<-0
  for (j in c2){
    jj<-jj+1
    rengen_01[ii,jj]<-mean((i < heikin_sa12 )&
                             (j < heikin_sa23 ) )
  }
}
rownames(rengen_01)<-c1
colnames(rengen_01)<-c2
round(rengen_01,3)

###  5.
lambda_sa12 <- extpo$lambda[,1]-extpo$lambda[,2]
lambda_sa23 <- extpo$lambda[,2]-extpo$lambda[,3]
...
c1<-c(0,0.1,0.2)
c2<-c(0,0.1,0.2)
rengen_02<-matrix(0,length(c1),length(c2));
ii<-0;
for (i in c1){
  ii<-ii+1
  jj<-0
  for (j in c2){
    jj<-jj+1
    rengen_02[ii,jj]<-mean((i < lambda_sa12 )&
                             (j < lambda_sa23 ) )
  }
}
rownames(rengen_02)<-c1
colnames(rengen_02)<-c2
round(rengen_02,3)

###  7.
hiritu_sa21 <- extbi$p[,2]-extbi$p[,1]
hiritu_sa32 <- extbi$p[,3]-extbi$p[,2]
...
PHC01(seq(0,3,0.02),extnorm$mu[,1]-extnorm$mu[,2],cc="gtc",byoga="yes",
      xlab="Difference between athlete 1 and athlete 2")
PHC01(seq(0,0.15,0.03),eta2,cc="gtc",byoga="no")
PHC01(seq(0,1,0.2),extnorm$mu[,1]-extnorm$mu[,2],cc="gtc",byoga="no")
