...
fit <- rstan::read_stan_csv(csrfit01$output_files())   # Conversion to stan format

########### Execution by rstan
#fit<-stan(model_code =logistic_reg_01,data=dataSet01,
#  pars=par, seed=1234, chains=5, warmup=1000,  iter=21000)
#save(fit,  file="./script/obje/out1701")
#load(file="./script/obje/out1701"); # Load pre-run stan( ) object

ext<-extract(fit, par);               # Extracting random numbers
...
csrfit02 <- mod02$sample(data = dataSet02,chains = 5,iter_sampling = 20000,
    iter_warmup = 1000,parallel_chains = 5,seed=1234)  # MCMC
fit02 <- rstan::read_stan_csv(csrfit02$output_files()) # Conversion to stan format

########### Execution by rstan
#fit02<-stan(model_code =binom_reg,data=dataSet02,
#  pars=par, seed=1234, chains=5, warmup=1000,  iter=21000)
#save(fit02,  file="./script/obje/out1702")
#load(file="./script/obje/out1702"); # Load pre-run stan( ) object

ext<-extract(fit02, par);               # Extracting random numbers
...
csrfitProspective <- modProspective$sample(data = dataSetProspective,chains = 5,iter_sampling = 20000,
    iter_warmup = 1000,parallel_chains = 5,seed=1234)  # MCMC
fitProspective <- rstan::read_stan_csv(csrfitProspective$output_files()) # Conversion to stan format

########### Execution by rstan
#fitProspective<-stan(model_code =meta_ana,data=dataSetProspective,
#  pars=par, seed=1234, chains=5, warmup=1000,  iter=21000)
#save(fitProspective,  file="./script/obje/out1703")
#load(file="./script/obje/out1703"); # Load pre-run stan( ) object

extProspective<-extract(fitProspective, par);               # Extracting random numbers

##### Table 17.10 Odds/EAP
print(fitProspective,digits_summary=3,prob=prob)
...
# "Boxplot of the Posterior Distribution of Odds Ratio for Each Prospective Study" (Not in textbook)
aa<-extProspective$Odds_r[,1]; for (i in 2:13){aa<-c(aa,extProspective$Odds_r[,i])};
boxplot(aa~rep(1:13,each=100000),outline=F,horizontal=T,cex.axis=1.2);
...
csrfitRetro <- modProspective$sample(data = dataSetRetro,chains = 5,iter_sampling = 20000,
    iter_warmup = 1000,parallel_chains = 5,seed=1234)  # MCMC
fitRetro <- rstan::read_stan_csv(csrfitRetro$output_files()) # Conversion to stan format

########### Execution by rstan
#fitRetro<-stan(model_code =meta_ana,data=dataSetRetro,
#  pars=par, seed=1234, chains=5, warmup=1000,  iter=21000)
#save(fitRetro,  file="./script/obje/out1704")
#load(file="./script/obje/out1704"); # Load pre-run stan( ) object

extRetro<-extract(fitRetro, par);               # Extracting random numbers

#### Table 17.12 Odds/EAP
print(fitRetro,digits_summary=3,prob=prob)
...
aa<-extRetro$Odds_r[,1]; for (i in 2:10){aa<-c(aa,extRetro$Odds_r[,i])}
...
###  3.
#load(file="./script/obje/out1704"); # Load pre-run stan( ) object
par<-c("Odds_r","Odds_r_mu","mu_b","sigma_b")                  
extRetro<-extract(fitRetro, par);               # Extracting random numbers               
